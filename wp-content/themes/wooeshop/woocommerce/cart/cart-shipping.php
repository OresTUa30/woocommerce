...
$calculator_text          = '';
?>
<tr class="woocommerce-shipping-totals shipping mb-3">
				...
				<?php foreach ( $available_methods as $method ) : ?>
					<li class = "list-group-item  p-1">
						...
						if ( 1 < count( $available_methods ) ) {
							...
							printf( '<input type="hidden" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method form-check-input" />', $index, esc_attr( sanitize_title( $method->id ) ), esc_attr( $method->id ) ); // WPCS: XSS ok.
						}
						printf( '<label class = "form-check-label ms-2" for="shipping_method_%1$s_%2$s">%3$s</label>', $index, esc_attr( sanitize_title( $method->id ) ), wc_cart_totals_shipping_method_label( $method ) ); // WPCS: XSS ok.
